...
const SeletorCliente = ({ selectedClientId, onClientSelect, value = null, onChange = null, required = false, showNewButton = true, showDialog = true }) => {
  ...
  useEffect(() => {
    ...
    if (value) {
      ...
      const cliente = clientes.find(cliente => cliente.id === selectedClientId);
      if (cliente) {
        setClienteSelecionado(cliente);
      }
    } else {
  ...
  const clienteItemTemplate = (cliente) => {
    return (
      <div className="cliente-item">
        ...
        {cliente.email && <div className="cliente-item-email">{cliente.email}</div>}
      </div>
    );
  };
  ...
  const clienteDialogFooter = (
    <React.Fragment>
      ...
      <Button label="Salvar" icon="pi pi-check" className="p-button-text" onClick={handleCriarCliente} />
    </React.Fragment>
  );
    ...
    <div className="seletor-cliente">
      <div className="p-field p-fluid">
        <div className="p-inputgroup">
          ...
          />
          {showNewButton && (
            <Button icon="pi pi-plus" className="p-button-success" onClick={abrirNovoDialog} tooltip="Novo cliente" />
          )}
        </div>
      </div>
        ...
        >
          <div className="p-fluid">
            ...
            <small className="p-mt-2">
              * Para adicionar mais informações do cliente, use a página Gerenciar Clientes.
            </small>
          </div>
        </Dialog>
      )}
